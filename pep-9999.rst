...
Optionally, authentication can be defined in a separate ``auth.toml``. This way, credentials can be fully or partially separated from repository URLs. The ``auth.toml`` takes precedence over ``config.toml``.
