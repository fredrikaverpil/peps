...
* Unix: ``$XDG_CONFIG_HOME/python`` with fallback to ``$HOME/.config/python/``
...
This logic explains the behavior::
...
Credit goes to djpinne for the merging code: https://stackoverflow.com/users/1827414/djpinne
...
Global unix location
''''''''''''''''''''

Originally, ``/etc/.config/python`` was proposed, but this was rejected in favor for ``/etc/python``.
